...
public partial class CPU
{
    ...
    {
        if (TheRegisters.PC >= MainMemory.MaxAddress)
            throw new PCHitMemoryLimit();
        ...
        var valueAddress = instructionAddress + instruction.ValueByteOffset;
        var oldFlags = TheRegisters.Main.F;
        ...
        try
        {
            switch (instruction.ID)
            {
                ...
                case Z80Instructions.InstructionID.LD_addrIXplus_d_n:
                    MainMemory.Poke(IXPlusD(MainMemory.Peek(valueAddress)), MainMemory.Peek(valueAddress + 1));
                    return instruction.TStateCount;
                case Z80Instructions.InstructionID.LD_addrIYplus_d_n:
                    MainMemory.Poke(IYPlusD(MainMemory.Peek(valueAddress)), MainMemory.Peek(valueAddress + 1));
                    return instruction.TStateCount;
                ...
                {
                    TheRegisters.CarryFlag = (TheRegisters.Main.A & 0x01) != 0x00;
                    TheRegisters.Main.A = (byte)((TheRegisters.Main.A >> 1) + ((TheRegisters.Main.A & 0x01) << 7));
                    TheRegisters.HalfCarryFlag = false;
                    TheRegisters.SubtractFlag = false;
                    return instruction.TStateCount;
                }
        ...
        {
            Debug.Assert(instruction.CheckFlagChanges(oldFlags, this));
        }
    }
}